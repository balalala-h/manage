      ...
      <el-col :span="12" :offset="4">
        ...
        >
          ...
          <el-form-item label="联系电话" prop="phone">
            <el-input v-model.number="formData.phone" :maxlength="maxlength"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
	//学习了表格中要新增一条或者多条数据时可以先用空数组缓存，用v-model进行双向数据绑定，在渲染的时候将新增数据的数组push进去的思路
	//删除表格的某条数据时可以先获取要被删除行数的index，然后通过splice(index,1)截取
	//更改数据后当想立即使用js操作新的视图时需要使用vue.nextTick()
	//组件库输入框默认输入的内容为字符串，可以通过v-model.number将其转换成数字类型，其底层原理是使用parseFloat()进行转换，但是最大长度的设置失效，
    //获取店铺分类
    async initData() {
      try {
    ...
    //选择优惠活动选项时触发，将输入的内容渲染到下面的表格的活动详情中
    selectActivity() {
