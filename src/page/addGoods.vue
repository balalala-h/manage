...
export default {
  ...
  methods: {
    ...
    async initData() {
      try {
        ...
        if (result.status == 1) {
          ...
          this.categoryForm.categoryList = result.category_list; //食品种类
        } else {
          console.log(result);
        }
      } catch (err) {
      ...
      this.$refs[categoryForm].validate(async valid => {
        if (valid) {
          ...
          try {
            ...
            if (result.status == 1) {
              this.initData();
              this.categoryForm.name = ""; //添加成功后清空
              this.categoryForm.description = "";
              this.showAddCategory = false; //添加成功后收起
              this.$message({
                type: "success",
                message: "添加成功"
              });
	...
	//上传文件之前的钩子，返回上传文件的参数，用于限制文件类型和文件大小
